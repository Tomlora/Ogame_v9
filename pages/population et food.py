...
level_residence = st.slider('Niveau résidence', 0,80,0)
level_ferme = st.slider('Niveau production food', 0, 80, 0)

vitesse_uni = st.slider('Vitesse economique', 1, 8, 1 )
marchand = st.checkbox('Classe alliance marchand ?')

if st.session_state['race'] == 'Human':
    ...
    factor_prod = 5
    bonus_prod_value = bonus_prod * factor_prod / 100
    
    st.write(bonus_prod_value)
    
elif st.session_state['race'] == 'Mecha':
...
elif st.session_state['race'] == 'Rock´tal':
...
if ratio_nourriture < 1:
    with st.expander('Informations supplémentaires'):
        ...
        if bonus_prod !=0:
            bonus_prod_value_next = (bonus_prod + 1) * factor_prod / 100
