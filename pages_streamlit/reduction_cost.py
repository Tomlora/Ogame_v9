...
def reduc_cost(data):
    ...
    st.session_state.race3 = st.selectbox('Selectionner la race de la planète', data['Lifeform'].unique())

    if st.session_state['race3'] == "Rocas":
    ...
    if st.session_state['race3'] == "Rocas" and st.session_state['type'] == 'Batiment':
    ...
    df_cout_mini = cost_cumul(st.session_state['race3'], name, level_act, level_max, bonus_reduc_f)
    df_cout = cost_cumul(st.session_state['race3'], name, level_act, level_max, 0)
    ...
    with st.expander('Voir cout du batiment de réduction'):
        if st.session_state['race3'] == "Rocas" and st.session_state['type'] == 'Batiment':
            st.write(cost_cumul(st.session_state['race3'], 'Monument rocheux', 0, bonus_reduc_f, 0))
